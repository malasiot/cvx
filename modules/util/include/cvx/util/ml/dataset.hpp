...
public:
    ...
    ShuffledDataset( const D &base, uint64_t n, cvx::util::RNG &rng ): base_(base) {
        assert(n <= base.size()) ;
        rng.sample(n, base.size(), perm_) ;
    }
...
public:
    ...
    KFold(uint32_t k, const D &base, cvx::util::RNG &rng ): ShuffledDataset<D>(base, base.size(), rng), K_(k) {}
