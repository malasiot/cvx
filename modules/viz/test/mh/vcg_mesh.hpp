...
/** @brief the type of a vcg face */
    class MyFace : public vcg::Face<MyUsedTypes, face::FFAdj,  face::Normal3f, face::VertexRef, face::BitFlags> { };
...
public:
    ...
    void concat(const float* verts, const uint* triangles, int nb_vert, int nb_face);
    ...
    void get_triangles(std::vector<uint>& triangles) {
        const int nb_triangles = this->face.size();
        triangles.clear();


        const int nb_vert = this->vert.size();
        std::map<MyMesh::ConstVertexPointer, int> vtxMap ;

        MyMesh::ConstVertexIterator vi = this->vert.begin();
        for(int i = 0; i < nb_vert; ++i, ++vi)
        {
             MyMesh::ConstVertexPointer p = &(*vi);
            vtxMap.emplace(p, i) ;
        }

        MyMesh::ConstFaceIterator fi = this->face.begin();
        for(int i = 0; i < nb_triangles; ++i, ++fi)
        {
            ...
            if( ! fi->IsD() ) {
                ...
                if ( vtxMap.find(v1) == vtxMap.end() ) continue ;
                if ( vtxMap.find(v2)== vtxMap.end() ) continue ;
                if ( vtxMap.find(v3)== vtxMap.end() ) continue ;
