...
bool TextLayoutEngine::shape(GlyphRun &line)
{
    ...
    for ( const auto & text_item : items ) {
        ...
        hb_buffer_add_utf16(buffer.get(), reinterpret_cast<const uint16_t *>(us_.getBuffer()), us_.length(), text_item.start_, static_cast<int>(text_item.end_ - text_item.start_));
...
bool TextLayoutEngine::itemize(int32_t start, int32_t end, vector<TextItem> &items) {
    ...
    if ( bidi_mode_ == cvx::gfx::TextDirection::Auto ) {
        if ( !itemizeBiDi(dir_runs, start, end) ) return false ;
    } else if ( bidi_mode_ == cvx::gfx::TextDirection::LeftToRight ) {
