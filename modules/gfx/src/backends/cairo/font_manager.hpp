...
public:
    ...
    cairo_font_face_t *queryFace(const std::string &family_name, cvx::gfx::FontStyle font_style, cvx::gfx::FontWeight font_weight) ;

    // Calls query face and if sucessfull scales the font found
    cairo_scaled_font_t *createFont(const cvx::gfx::Font &font) ;

private:

    FontManager() = default ;

    static std::string font_face_key(const std::string &family_name, cvx::gfx::FontStyle font_style, cvx::gfx::FontWeight font_weight) ;
